    ...
    <div className="flex justify-between items-center w-full h-20 text-bearbrown bg-sand fixed px-4 text-xl">
      ...
      <ul className='hidden md:flex'>
        {links.map(({ id, link}) => (
           <li
           key={id}
           className='text-xl text-bearbrown hover:scale-105 duration-200 px-4 cursor-pointer hover:line-through'>
      ...
      <div onClick={() => setNav(!nav)} className='cursor-pointer pr-4 z-10 text-bearbrown md:hidden'>
      ...
      {nav && (
        ...
        {links.map(({ id, link }) => (
            <li
            key={id}
            className='px-4 text-2xl text-bearbrown py-4 cursor-pointer'>
